...
from src.services.client_service import getSession, logout
    ...
    try:
        ...
        with pd.ExcelWriter(draft, engine="openpyxl") as writer:
            ...
            if (
                ...
                executed_bast_df.to_excel(
                    writer, sheet_name="EXECUTED-BAST", index=False
                )
            if (
                report_bast_df is not None
                and not report_bast_df.dropna(how="all").empty
            ):
            ...
            if (
                ...
                executed_close_df.to_excel(
                    writer, sheet_name="EXECUTED-CLOSE", index=False
                )
            if (
                report_close_df is not None
                and not report_close_df.dropna(how="all").empty
            ):
